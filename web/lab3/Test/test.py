...
def init():
    ...
    ret_code = run_command(
        ["/bin/cp", "-rf", f"{HANDIN_DIR}{TEST_FILE}", "./"])
    if ret_code:
    ...
    for __ in test_cases:
        ret_code = run_command(
            ["/bin/cp", "-rf", f"{TESTCASE_DIR}{__}", f"{TEST_FILE}/"])
        if ret_code:
    ...
    atexit.register(lambda: run_command(
        ["/bin/bash", "-c", f"cd ..; rm -rf {TEST_FILE}"]))
...
def check_and_make():
    ...
    files = os.listdir(".")
    if AGENT in files:
        run_command(["/bin/rm", "-f", AGENT])
    if ROUTER in files:
        run_command(["/bin/rm", "-f", ROUTER])
    ...
    ret_code = run_command(
        ["/bin/bash", "-c", f"ls -R | grep -F {AGENT_C}"])  # disable regex
    if ret_code:
    ...
    ret_code = run_command(
        ["/bin/bash", "-c", f"ls -R | grep -i {MAKEFILE}"])  # ingore case
    if ret_code:
        log_and_exit(f"Error: file '{MAKEFILE}' missing.")
    # make
    ret_code = run_command([f"make"])
    if ret_code:
